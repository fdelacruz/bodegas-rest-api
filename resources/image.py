...
class ImageUpload(Resource):
    ...
    def post(cls):
        ...
        It uses JWT to retrieve user information and then save the image to the
        user's folder.
...
class Image(Resource):
    ...
    def get(cls, filename: str):
        """
        Returns the requested image if it exists. Looks up inside the logged in
        user's folder.
        """
        user_id = get_jwt_identity()
...
class AvatarUpload(Resource):
    ...
    def put(cls):
        """
        This endpoint is user to upload user avatars. All avatars are named
        after the user's ID. Something like: user_{id}.{ext}
